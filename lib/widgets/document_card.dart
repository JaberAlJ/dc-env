import 'package:dc_env/change_notifiers/new_document_controller.dart';
import 'package:dc_env/change_notifiers/documents_provider.dart';
import 'package:dc_env/core/dialogs.dart';
import 'package:flutter/material.dart';
import 'package:font_awesome_flutter/font_awesome_flutter.dart';
import 'package:provider/provider.dart';

import '../core/constants.dart';
import '../core/utils.dart';
import '../enums/order_option.dart';
import '../models/DCDocument.dart';
import '../pages/new_or_edit_document_page.dart';
import 'document_tag.dart';

class DocumentCard extends StatelessWidget {
  const DocumentCard({
    required this.document,
    required this.isInGrid,
    super.key,
  });

  final DCDocument document;
  final bool isInGrid;

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => ChangeNotifierProvider(
              create: (_) => NewDocumentController()..document = document,
              child: const NewOrEditDocumentPage(
                isNewDocument: false,
              ),
            ),
          ),
        );
      },
      child: Container(
        decoration: BoxDecoration(
          color: white,
          border: Border.all(
            color: primary,
            width: 2,
          ),
          borderRadius: BorderRadius.circular(12),
          boxShadow: [
            BoxShadow(
              color: primary.withOpacity(0.5),
              offset: const Offset(4, 4),
            ),
          ],
        ),
        padding: const EdgeInsets.all(12),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (document.title != null) ...[
              Text(
                document.title!,
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: const TextStyle(
                  fontWeight: FontWeight.bold,
                  fontSize: 16,
                  color: gray900,
                ),
              ),
              const SizedBox(height: 4),
            ],
            if (document.tags != null) ...[
              SingleChildScrollView(
                scrollDirection: Axis.horizontal,
                child: Row(
                  children: List.generate(
                    document.tags!.length,
                    (index) => DocumentTag(label: document.tags![index]),
                  ),
                ),
              ),
              const SizedBox(height: 4),
            ],
            if (document.content != null)
              isInGrid
                  ? Expanded(
                      child: Text(
                        document.content!,
                        style: const TextStyle(color: gray700),
                      ),
                    )
                  : Text(
                      document.content!,
                      maxLines: 3,
                      overflow: TextOverflow.ellipsis,
                      style: const TextStyle(color: gray700),
                    ),
            if (isInGrid) const Spacer(),
            Row(
              children: [
                Selector<DocumentsProvider, OrderOption>(
                  selector: (_, documentsProvider) => documentsProvider.orderBy,
                  builder: (_, orderBy, __) => Text(
                    toShortDate(orderBy == OrderOption.dateModified
                        ? document.dateModified
                        : document.dateCreated),
                    style: const TextStyle(
                      fontSize: 12,
                      fontWeight: FontWeight.w600,
                      color: gray500,
                    ),
                  ),
                ),
                const Spacer(),
                GestureDetector(
                  onTap: () async {
                    final shouldDelete = await showConfirmationDialog(
                            context: context,
                            title: 'Do you want to delete this document?') ??
                        false;

                    if (shouldDelete && context.mounted) {
                      context.read<DocumentsProvider>().deleteDocument(document);
                    }
                  },
                  child: const FaIcon(
                    FontAwesomeIcons.trash,
                    color: gray500,
                    size: 16,
                  ),
                ),
              ],
            ),
          ],
        ),
      ),
    );
  }
}
